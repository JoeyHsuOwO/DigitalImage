   ...
   for (i=0; i < biHeight; i++)
   {
		...
		for (j=0; j < biWidth; j++)
		{
			for (i1 = i*RATIO; i1 < i*RATIO+RATIO; i1++)
				...
				{
					new_data[i1*biWidth*RATIO+j1]=(data[k]*(i*RATIO+RATIO-i1)*(j*RATIO+RATIO-j1)+
						data[k+biWidth+1]*(i1-i*RATIO)*(j1-j*RATIO)+
						data[k+1]*(i*RATIO+RATIO-i1)*(j1-j*RATIO)+
						data[k+biWidth]*(i1-i*RATIO)*(j*RATIO+RATIO-j1))/(RATIO*RATIO);
				}
